...
def train_boosted_knn(X_train, y_train, best_knn):
    """
...
def main_forest_knn(dataset_name):
    ...
    for method, n_components, n_classifiers in itertools.product(methods, n_components_list, n_classifiers_list):
        ...
        for i in range(n_classifiers):
            ...
            best_knn = grid_search_knn(X_projected, y_sample)
            # Train boosted kNN model
            boosted_knn = train_boosted_knn(X_projected, y_sample, best_knn)
            classifiers.append(boosted_knn)
            X_test_samples.append(X_test_projected)
        
        evaluate_and_save_results(classifiers, X_test_samples, y_test, method, n_components, None, n_classifiers, output_file)
