...
# ForestKNN add Balancing to data
# results are bad, no longer looking at this
...
def analyze_dataset(X, y):
    print("Dataset dimensions before balancing:")
...
def main(dataset_name):
    ...
    # Analyze dataset before balancing
    analyze_dataset(X_train, y_train)
    
    # Apply SMOTE and ENN to balance the dataset
    X_train_balanced, y_train_balanced = apply_smote_enn(X_train, y_train)
    
    # Analyze balanced dataset
    analyze_dataset(X_train_balanced, y_train_balanced)
    ...
    output_file = f"{dataset_name}_forestknn_step3_v3.txt"
    with open(output_file, "w") as f:
        ...
        for num_trees, k, sample_size, feature_size in product(num_trees_list, k_list, sample_size_list, feature_size_list):
            ...
            forest_knn.fit(X_train_balanced, y_train_balanced)
